...
logit ther_change_censor i.matage_group i.ethnicity i.imd  ib1.therapy i.his_exac i.review_bef  i.eos_high i.atopy  i.anx_dep  i.bmi_cat i.multigravida i.smoking_dur, or
...
logit ther_change_censor i.matage_group i.ethnicity i.imd i.bmi_cat i.smoking_dur ib2.therapy i.his_exac i.review_bef  i.eos_high i.atopy  i.anx_dep   i.multigravida , or
...
coefplot increased||, eform  /// 
xtitle(Odds ratio (log scale), size(vsmall)) ///
xscale(log) ///
xlabel(0.5 "0.5" 1 "1" 2 "2" 3.0 "3.0" , labsize(vsmall)) ///
...
scheme(sj) ///
title("Increased ICS use", margin(0 0 5 0))


coefplot decreased||, eform  /// 
xtitle(Odds ratio (log scale), size(vsmall)) ///
xscale(log) ///
xlabel(0.5 "0.5" 1 "1" 2 "2" 3.0 "3.0" , labsize(vsmall)) ///
xline(1, lcolor(black) lwidth(thin) lpattern(dash))  ///
base ///
coeflabels (, labsize(vsmall)) ///
headings(2.therapy="{bf: Asthma medication}" 0.his_exac="{bf:Asthma exacerbations}"  0.ther_change_censor="{bf:Change in ICS}" 0.eos_high="{bf:Blood eosinophil count(x10{sup:9}/L)}" 1.ethnicity="{bf:Ethnicity}" 1.imd="{bf: IMD}" 0.bmi_cat="{bf:BMI}" 1.anx_dep="{bf: Anxiety/Depression}" 1.atopy="{bf: Atopy}" 1.matage_group="{bf: Maternal age group}" 0.multigravida="{bf: Multigravida}" 1.smoking_dur="{bf: Smoking during pregnancy}" 0.review_before="{bf:Annual asthma review}") ///
byopts(cols(1)) /// 
xsize(1.8) ///
msymbol(square) /// 
msize(0.5) ///
...
drop(_cons 0.anx_dep 0.atopy) ///
scheme(sj) ///
title("Decreased ICS use", margin(0 0 5 0))


graph combine increase decrease
